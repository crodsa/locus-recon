...
def run_case(case: dict, bait_path: str, outdir: str, tools: dict) -> dict:
    ...
    db = os.path.join(workdir, "assembly_db")
    subprocess.run(
    ...
    bases_unchanged = len(extracted) == expected_bp
    ...
    qc_with = assess_allele_quality(
        extracted, PROFILE_CACHE["profile"], 100.0,
        100.0 * len(extracted) / best["qlen"], "benchmark",
        ...
        exact_known_allele=(len(extracted) == best["qlen"]),
    )
    ...
    row = {
        ...
        "reconstruction_bases_unchanged": bases_unchanged,
...
def main() -> int:
    ...
    parser.add_argument("--bait", required=True,
    ...
    rows = [run_case(case, bait_path, args.outdir, tools) for case in cases]
    ...
    summary = {
        ...
        "cases_reconstruction_unchanged": sum(
            1 for r in rows if r.get("reconstruction_bases_unchanged")),

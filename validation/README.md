...
The compact files in `reference_results/` were generated with Locus-Recon
1.0.0, BWA-MEM2 2.2.1, samtools 1.22.1, SPAdes 4.2.0, and BLAST+ 2.16.0+ on
Linux x86_64. The execution record, including the environment and the outcome
...
| Depth series | Exact allele at 3x, 5x, 10x and 20x, reported at `SUSPECT`, `SUSPECT`, `LOW` and `MEDIUM` |
| Controlled mixtures | Detected at 5% (at the two-site minimum), 10%, 20%, and 50% |
...
contemporary LIBA-6656 local assembly graph with the Locus-Recon 1.0.0
...
paths are combinatorial proposals through nine variable regions; they are not
512 accepted alleles and still require the competitive-read, per-base,
polishing and context evidence deposited with the publication package.
...
The paralog control returns a sequence that is not exactly right. At 110.8x
mean remap depth and 100.0% breadth the run is reported as `SUCCESS`, yet the
returned allele is 99.01% identical to the designated truth, which over 507 bp
is five mismatches. The target and its paralog differ at fourteen positions and
at no other position, so those five errors sit on discriminating sites: the
consensus took the paralog base. The evidence carries the warning that the
status does not, since all fourteen discriminating positions are credible
bidirectional mixed sites, at a median minor fraction of 0.472, the ambiguity
flag is raised, and the QC verdict is `SUSPECT`. A locus with a near-identical
paralog is read from the evidence table, never from `status` alone.

The low-evidence modes fail in the safer direction. At nominal 3x and 5x the
allele is returned and exact, but it is held at `SUSPECT` with
`LOW_REMAP_DEPTH`, and the negative control yields no candidate contig, so
scarcity of evidence produces a withheld result or silence, never an accepted
allele.
...
Publication-facing analyses are executable from accession-level inputs.
Shared helpers in `validation/common.py` fail on missing inputs or non-zero
commands and record SHA-256 checksums, the package version with a SHA-256
digest of its sources, the Git commit when there is one, parameters, and tool
versions.
...
interval supplies the aggregate depth and dosage estimate. The workflow
requires all seven runs and evaluates the two published qPCR
...
design and are therefore a biological demonstration, not an independent
exact-copy performance estimate.
